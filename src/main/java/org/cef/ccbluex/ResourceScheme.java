...
public class ResourceScheme implements IScheme {
    ...
    public SchemePreResponse processRequest(String url) {
        ...
        if(is == null) {
            LoggerUtil.addTerminalMessage("Resource " + url + " NOT found!");
    ...
    public static void loadMimeTypeMapping() {
        ...
        try {
            BufferedReader br = new BufferedReader(new InputStreamReader(ResourceScheme.class.getResourceAsStream("/assets/minecraft/slice/cef/mime.types")));
            ...
            LoggerUtil.addTerminalMessage("[Mime Types] Error while parsing \"" + line + "\" at line " + cLine + " : " + e.getMessage());
            e.printStackTrace();
        }

        LoggerUtil.addTerminalMessage("Loaded " + mimeTypeMap.size() + " mime types");
    }
